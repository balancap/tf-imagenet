...
VALIDATION_TARBALL="${OUTDIR}../${VALIDATION_TARBALL}"
...
TRAIN_TARBALL="${OUTDIR}../${TRAIN_TARBALL}"
...
while read SYNSET; do
  ...
  SYNSET_FILE="${OUTPUT_PATH}${SYNSET}.tar"
  # Uncompress into the directory.
  # tar xf "${TRAIN_TARBALL}" "${SYNSET_FILE}"
  tar xf "${SYNSET_FILE}" -C "${OUTPUT_PATH}/${SYNSET}/"
  rm -f "${SYNSET_FILE}"
