...
NASNET_DATA_FORMAT = 'NCHW'

# =========================================================================== #
# NASNet models.
# =========================================================================== #
class NASNetV1Small(abstract_model.Model):
    """Nasnet, small model."""
    def __init__(self):
        self.scope = 'NASNetV1Small'
        super(NASNetV1Small, self).__init__(self.scope, 224, 32, 0.005)
    def forward(self, inputs, num_classes, data_format, is_training):
        global NASNET_DATA_FORMAT
        NASNET_DATA_FORMAT = data_format
        sc = nasnet_mobile_arg_scope(
            weight_decay=4e-5,
            batch_norm_decay=0.9997,
            batch_norm_epsilon=1e-3)
        with slim.arg_scope(sc):
            logits, end_points = build_nasnet_mobile(
                inputs, num_classes, is_training=is_training, final_endpoint=None)
            return logits, end_points

class NASNetV1Large(abstract_model.Model):
    """Nasnet, large model."""
    def __init__(self):
        self.scope = 'NASNetV1Large'
        super(NASNetV1Large, self).__init__(self.scope, 331, 32, 0.005)
    def forward(self, inputs, num_classes, data_format, is_training):
        global NASNET_DATA_FORMAT
        NASNET_DATA_FORMAT = data_format
        sc = nasnet_large_arg_scope(
            weight_decay=5e-5,
            batch_norm_decay=0.9997,
            batch_norm_epsilon=1e-3)
        with slim.arg_scope(sc):
            logits, end_points = build_nasnet_large(
                inputs, num_classes, is_training=is_training, final_endpoint=None)
            return logits, end_points
...
def _cifar_config(is_training=True):
    ...
    return tf.contrib.training.HParams(
        ...
        data_format=NASNET_DATA_FORMAT,
...
def build_nasnet_cifar(images, num_classes, is_training=True):
