...
# MobileNet model.
# =========================================================================== #
class HexMobileNetV1(abstract_model.Model):
    """MobileNetV1 model.
    """
    def __init__(self, ksize=5, regularize_depthwise=False):
        self.scope = 'HexMobilenetV1'
        self.ksize = ksize
        self.regularize_depthwise = regularize_depthwise
        super(HexMobileNetV1, self).__init__(self.scope, 224, 32, 0.005)

    def forward(self, inputs, num_classes, data_format, is_training):
        sc = hex_mobilenet_v1_arg_scope(
        ...
        with slim.arg_scope(sc):
            logits, end_points = hex_mobilenet_v1(
                inputs,
                ...
                conv_defs=hex_mobilenet_v1_def(self.ksize),
...
HexDepthSepConv = namedtuple('HexDepthSepConv', ['kernel', 'stride', 'depth'])

def hex_mobilenet_v1_def(ksize=5):
    ...
    _CONV_DEFS = [
        ...
        HexDepthSepConv(kernel=[ksize, ksize], stride=1, depth=64),
        HexDepthSepConv(kernel=[ksize, ksize], stride=2, depth=128),
        HexDepthSepConv(kernel=[ksize, ksize], stride=1, depth=128),
        HexDepthSepConv(kernel=[ksize, ksize], stride=2, depth=256),
        HexDepthSepConv(kernel=[ksize, ksize], stride=1, depth=256),
        HexDepthSepConv(kernel=[ksize, ksize], stride=2, depth=512),
        HexDepthSepConv(kernel=[ksize, ksize], stride=1, depth=512),
        HexDepthSepConv(kernel=[ksize, ksize], stride=1, depth=512),
        HexDepthSepConv(kernel=[ksize, ksize], stride=1, depth=512),
        HexDepthSepConv(kernel=[ksize, ksize], stride=1, depth=512),
        HexDepthSepConv(kernel=[ksize, ksize], stride=1, depth=512),
        HexDepthSepConv(kernel=[ksize, ksize], stride=2, depth=1024),
        HexDepthSepConv(kernel=[ksize, ksize], stride=1, depth=1024)
    ]
    return _CONV_DEFS


def hex_mobilenet_v1_base(inputs,
    ...
    if conv_defs is None:
        conv_defs = hex_mobilenet_v1_def()
    ...
    with tf.variable_scope(scope, 'MobilenetV1', [inputs]):
        with slim.arg_scope([slim.conv2d, slim.separable_conv2d], padding='SAME'):
            ...
            for i, conv_def in enumerate(conv_defs):
                ...
                # Hex Depth sep. conv2d.
                elif isinstance(conv_def, HexDepthSepConv):
                    end_point = end_point_base + '_depthwise'
                    # Hex depthwise.
...
def hex_mobilenet_v1(inputs,
    ...
    with tf.variable_scope(scope, 'MobilenetV1', [inputs], reuse=reuse) as scope:
        with slim.arg_scope([slim.batch_norm, slim.dropout],
                            is_training=is_training):
            net, end_points = hex_mobilenet_v1_base(inputs, scope=scope,
                                                min_depth=min_depth,
...
hex_mobilenet_v1.default_image_size = 224
...
def hex_mobilenet_v1_arg_scope(is_training=True,
                                data_format='NHWC',
